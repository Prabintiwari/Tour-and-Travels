...
export const createTourFAQSchema = z.object({
  tourId: z.string().regex(/^[0-9a-fA-F]{24}$/, 'Invalid tour ID').openapi({
  ...
  answer: z.string().min(10).max(2000).openapi({
    ...
    example: 'You can cancel up to 7 days before the tour starts for a full refund.'
  }),
...
export const updateTourFAQSchema = z.object({
  ...
  answer: z.string().min(10).max(2000).optional().openapi({
    description: 'FAQ answer',
    example: 'Full refund available for cancellations made 7+ days in advance.'
  }),
...
export const tourFAQResponseSchema = z.object({
  ...
  answer: z.string().openapi({ example: 'Comfortable shoes, water bottle, and sunscreen.' }),
  isActive: z.boolean().openapi({ example: true }),
  createdAt: z.string().openapi({ example: '2024-01-01T00:00:00.000Z' }),
  updatedAt: z.string().openapi({ example: '2024-01-01T00:00:00.000Z' })
}).openapi('TourFAQResponse');

export const tourFAQsListResponseSchema = paginatedResponse(tourFAQResponseSchema).openapi('TourFAQsListResponse');

export type CreateTourFAQInput = z.infer<typeof createTourFAQSchema>;
export type UpdateTourFAQInput = z.infer<typeof updateTourFAQSchema>;